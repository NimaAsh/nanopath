...
- Alliance login nodes have internet but no GPU access; compute nodes have GPU access but no internet. `/scratch` is per-cluster (Rorqual/Trillium/Nibi do not share it), so on **each** cluster you train on, set up its own venv and run `prepare.py ... download=True` once on its login node — first `export TORCH_HOME=/scratch/$USER/nanopath/cache/torch HF_HOME=/scratch/$USER/nanopath/cache/hf` so DINOv2 weights land where compute nodes read them (not `~/.cache`) — then verify with `download=False` before submitting GPU jobs.
